...
function useSkill() {
  ...
  if (selectedCat.name === "ATTACK") {
    ...
    if (humans.length > 0) {
      ...
      for (let spread = -0.3; spread <= 0.3; spread += 0.3) {
...
function selectCat(catType) {
  ...
  player.speed = selectedCat.speed;
  player.emoji = selectedCat.emoji;
...
function draw() {
  ...
  if (dashMode) {
    ...
    ctx.fillText("DASH!", player.x - 10, player.y - 30);
